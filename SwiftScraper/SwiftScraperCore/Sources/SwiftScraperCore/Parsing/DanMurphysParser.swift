    ...
    public func parse(html: String) -> [ParsedBeer] {
        ...
        if let carousel = try? doc.select("dd-product-carousel").first() {
            ...
            for root in dmTiles {
                ...
                let cardText = (try? innerTextApproximation(from: root)) ?? ""
                rows.append(DomProductRow(breweryLine: breweryLine, subtitle: subtitle, href: href, cardText: cardText))
            }
        }
        ...
        for card in gridCards {
            ...
            let cardText = (try? innerTextApproximation(from: card)) ?? ""
            rows.append(DomProductRow(breweryLine: breweryLine, subtitle: subtitle, href: href, cardText: cardText))
        }
...
/// SwiftSoup's `text()` joins nodes with spaces; Playwright's `innerText` breaks on block elements. We approximate that so `parsePricesFromCardText` sees the same logical lines as `danmurphys.ts`.
private func innerTextApproximation(from element: Element) throws -> String {
    try element.html().htmlFragmentToInnerTextLike()
}

/// Matches `new URL(href)` then `` `${u.origin}${u.pathname}` `` in `danmurphys.ts`.
private func canonicalProductURL(_ href: String) -> String? {
    let absolute = href.hasPrefix("http") ? href : "https://www.danmurphys.com.au\(href)"
...
private func resolveBreweryAndName(_ row: DomProductRow) -> (brewery: String, name: String)? {
    var brewery = row.breweryLine.trimmingCharacters(in: .whitespacesAndNewlines)
    var name = cleanBeerName(row.subtitle)
    guard !brewery.isEmpty else { return nil }
    if name.isEmpty {
        if let slug = parseBreweryAndNameFromProductHref(row.href) {
            brewery = slug.brewery
            name = slug.name
        }
    }
    if !name.isEmpty, !isPlausibleProductName(name) { return nil }
    return (brewery, name)
}

private func cleanBeerName(_ raw: String) -> String {
...
private func parseBreweryAndNameFromProductHref(_ href: String) -> (brewery: String, name: String)? {
    let urlString = href.hasPrefix("http") ? href : "https://www.danmurphys.com.au\(href)"
    ...
    let name = cleanBeerName(before.dropFirst().joined(separator: " ").capitalizeWords())
    guard !brewery.isEmpty, !name.isEmpty else { return nil }
    return (brewery, name)
}

private func isPlausibleProductName(_ name: String) -> Bool {
...
private func inferVessel(_ text: String) -> VesselType? {
...
private func extractSizeMl(_ text: String) -> Int? {
...
private struct MutablePrice {
    var price: Double
    var quantity: Int
    var memberOffer: Bool
}

private func parsePricesFromCardText(_ text: String) -> [MutablePrice] {
    let lines = cardTextLines(text)
    var entries: [MutablePrice] = []
    for i in lines.indices {
        ...
        for price in dollarAmounts(in: line) {
            ...
            entries.append(MutablePrice(price: price, quantity: max(1, quantity), memberOffer: memberOffer))
        }
    }
...
private func cardTextLines(_ text: String) -> [String] {
...
private func reconcileMemberOffersForSameQuantity(_ entries: inout [MutablePrice]) {
    var byQty: [Int: [Int]] = [:]
...
private func reconcileMemberOffersMixedQuantity(_ entries: inout [MutablePrice], fullCardText: String) {
    ...
    func perUnit(_ e: MutablePrice) -> Double { e.price / Double(e.quantity) }
    let aPu = perUnit(a)
    let bPu = perUnit(b)
...
private func dedupePriceEntries(_ entries: [MutablePrice]) -> [MutablePrice] {
    var seen = Set<String>()
    var out: [MutablePrice] = []
    for p in entries {
